...
from app.utils.query_normalization import normalize_measurements
...
class MatchingService:
    def apply(self, query: str, results: list[NormalizedResult]) -> list[NormalizedResult]:
        ...
        for result in results:
            title_tokens = self._tokenize(normalize_measurements(result.title))
            ...
            if result.brand and result.brand.lower() in normalized_query.lower():
                score = min(100, score + 8)
                why_parts.append("brand matched query")
            if result.sku and result.sku.lower() in normalized_query.lower():
